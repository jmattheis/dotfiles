...
local plugins = {
  ...
  { -- format everything
    ...
    config = function() vim.g.neoformat_rust_rustfmt = {exe = 'rustfmt', args = {'--edition 2021'}, stdin = 1} end,
  },
  { -- theme
    'morhetz/gruvbox',
    config = function()
  ...
  { -- show spaces / tabs everywhere
    ...
    config = function() require'ibl'.setup {scope = {enabled = false}, indent = {char = {'│'}}} end,
  },
  ...
  },
  {
    ...
    config = function()
      require('nvim-tree').setup({view = {width = 44}, renderer = {icons = {show = {git = false, modified = false}}}})
  ...
  { -- lsp
    ...
    config = function()
      ...
      local on_attach = function(client, bufnr)
        ...
        buf_keymap('n', '<leader>ad', '<cmd>lua vim.lsp.buf.definition()<CR>', {silent = true, noremap = true})
        buf_keymap('n', '<leader>aa', ':CodeActions<CR>', {silent = true, noremap = true})
        buf_keymap('n', '<leader>aF', '<cmd>lua vim.lsp.buf.formatting()<CR>', {silent = true, noremap = true})
        buf_keymap('n', '<leader>dl', '<cmd>lua vim.diagnostic.open_float({bufno = 0})<CR>',
                   {silent = true, noremap = true})
        buf_keymap('n', '<leader>dn', '<cmd>lua vim.diagnostic.goto_next()<CR>', {silent = true, noremap = true})
  ...
  callback = function(opts)
    autocmd('BufWinEnter', {
      ...
      callback = function()
        ...
        if ft:match('gitcommit') or ft:match('gitrebase') then return end
